...
public class WindowsNativeBridge extends NativeBridge {
  ...
  public synchronized IcmpPingResponse executePingRequest (final IcmpPingRequest request) {
    ...
    try {
      ...
      // timeout must be set, otherwise IcmpSendEcho fails with a generic "Last error: 87"
      // https://msdn.microsoft.com/en-us/library/windows/desktop/ms681382(v=vs.85).aspx
      //   ERROR_INVALID_PARAMETER
      //   87 (0x57)
      //   The parameter is incorrect.
      // and, btw, we have NO idea which paramter is bad, so we have to just test them all
      final int timeout2 = timeout > 0 ?
        timeout :
        60 * 1000;
      ...
      if (returnCode == 0) {
        ...
        // call failed
        final String errorMessage = "IcmpSendEcho failed. Last error: " + lastError;
